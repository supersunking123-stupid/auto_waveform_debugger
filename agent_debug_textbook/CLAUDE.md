...
**For explicitly authorized parallel debug runs**, use `agent_debug_textbook/10_PARALLEL_DEBUG_ORCHESTRATION.md`: the Orchestrator pre-creates per-agent sessions, warms the shared waveform backend once, strips orchestration-only directives from the user request, launches independent Debugger agents with the resulting serial-mode debug task and distinct explicit session names, waits for all conclusions or the configured timeout, and produces a report listing every agent's conclusion.
