...
If the symptom includes architecturally relevant unknown (`X`) data or control, route through `09_X_TRACING.md` first. Playbook 09 owns the X-specific creator-block and subsystem-boundary audit; when you return to Playbook 04, reuse that completed evidence table instead of repeating the same proof unless the harmful interval, activation edge, active path, controlling context, or creator candidate changed.
...
- **Harmful `X` trigger:** If the symptom includes architecturally relevant unknowns on data or control, route through `09_X_TRACING.md` first so you isolate the likely creator block before doing deep internal tracing. Reuse Playbook 09's evidence table when you return here instead of repeating the X-specific boundary audit.
