...
- **Harmful `X` trigger:** If the symptom includes architecturally relevant unknowns on data or control, route through `09_X_TRACING.md` first so you isolate the likely creator block before doing deep internal tracing.
- **Creator-block gate for harmful `X`:** For `X` bugs, the final exact RTL statement must come from the first creator block, not the first exact line on the bad cone. Do not descend into local source or report a final line until you have shown the current block has harmful `X` outputs but no harmful `X` inputs, and whether the boundary owner is a child instance, sibling, local logic, parent-glue, or top-level. If a child instance owns the harmful boundary, move into that child, restart from the child's ingress boundary, and do not trace child outputs yet. If Playbook 09 already completed this creator-block gate for the same harmful interval, activation edge, and current creator candidate, reuse it instead of repeating the same proof.
- **Subsystem-boundary gate for harmful `X`:** If the harmful boundary crosses a mapped subsystem boundary, keep the frontier at that subsystem layer first. Do not jump directly to a deep internal child returned by structural trace. Classify the subsystem egress and the relevant subsystem ingress groups; descend inside the subsystem only if the subsystem egress is harmful while subsystem ingress is clean or gated off. This subsystem-boundary gate overrides the generic child-owner shortcut; even if the wrapper looks pass-through on the current path, ingress classification is still mandatory. However, if that same subsystem boundary was already audited for the same harmful interval, activation edge, and current harmful path, reuse the existing audit and continue downward instead of bouncing back to the subsystem wrapper.
