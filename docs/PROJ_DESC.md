...
- `agent_debug_textbook/10_PARALLEL_DEBUG_ORCHESTRATION.md` defines the recommended multi-agent workflow: the Orchestrator pre-creates per-agent sessions, warms the shared waveform backend once, strips orchestration-only directives from the user request, launches independent Debugger agents with the resulting serial-mode debug task, waits for all conclusions or a default 30-minute timeout, then reviews every conclusion and generates a final report.
