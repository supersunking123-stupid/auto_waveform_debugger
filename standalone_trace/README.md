...
```bash
build/rtl_trace compile --db rtl_trace.db [slang source args...]
build/rtl_trace trace --db rtl_trace.db --mode drivers --signal top.u0.sig
build/rtl_trace trace --db rtl_trace.db --mode loads   --signal top.u0.sig
```

说明：
- `compile` 阶段会解析/展开 RTL 并生成离线数据库（默认 `rtl_trace.db`）。
- `trace` 阶段只查询数据库，不会再次解析 RTL。
- 数据库当前版本为 `RTL_TRACE_DB_V3`（可读取旧版 V1/V2）。
